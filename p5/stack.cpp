...
void Stack::operator=(Stack const &other) {
    ...
    if (other.size == 0) {
        ...
        // create new head
        head                  = new StackNode;
        StackNode *copy_iter  = head;
        StackNode *other_iter = other.head;
...
//       -throws an error if a node is not popped
int Stack::pop() {
    int popped_val = -1;
    if (size > 0) {
        StackNode *temp = head->next;
        ...
        throw std::runtime_error("Stack is empty, runtime error thrown.");
    }
    size--;
    return popped_val;
}
